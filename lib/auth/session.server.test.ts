...
vi.mock("@/lib/auth/server", () => ({
  getOrCreateAuth: () => ({ api: { getSession: mocks.getSession } }),
}));
...
test("requireSession redirects to the invoking page without leading slash in query parameter", async () => {
  ...
  await expect(requireSession()).rejects.toMatchObject({
    destination:
      "/sign-in?callbackUrl=users%3Ftab%3Dmembers%26filter%3Dname%253Aalice",
  });
});

test("requireSession uses request URL metadata without leading slash for server actions", async () => {
  vi.mocked(headers).mockResolvedValue(
  ...
  await expect(requireSession()).rejects.toMatchObject({
    destination: "/sign-in?callbackUrl=users%3Ftab%3Dmembers",
  });
});
