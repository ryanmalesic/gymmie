...
import updateLocation, {
  spec,
} from "@/domain/locations/commands/update.command";
...
test("updateLocation command spec and execution", async () => {
  expect(spec.name).toBe("UpdateLocation");

  const mockPrisma = {
    location: {
      update: vi
        .fn()
        .mockResolvedValue({ ...location, name: "Ironworks West" }),
    },
  };
  ...
  const result = await updateLocation(
    { id: "loc_1", name: "Ironworks West" },
    context,
  );
  expect(result.name).toBe("Ironworks West");
  expect(mockPrisma.location.update).toHaveBeenCalledOnce();
});

test("updateLocation authorize allows only the owner", () => {
